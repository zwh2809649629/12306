...
>
> 最后更新：2026-09-25 · Python 3.12.14 · Flask 3.1.3
...
| 场景 | 单条路线 | 多区间 | 实现 | 状态 |
|---|---|---|---|---|
| 总览任务表、任务列表卡片、任务详情“行程”、新建任务摘要 | `起点➜终点` | `、` | `routeDetailText()` 直接文本 | 已统一 |
| 新建任务推导区、车次卡片、关联任务弹窗、车票查询标题、经停站弹窗 | `起点➜终点` | 按组件需要 | `routeHtml()` + `.route-arrow::before` | 交互组件内部已统一 |
| 后端任务名、普通业务提示 | `起点 至 终点` | 按语义文本 | 后端字符串拼接 / `routeText()` | 不改为视觉路线 |
| 查询日志中的任务区间 | `[起点➜终点]`；带车次为 `[车次(起点➜终点)]` | 车次具体区间、任务匹配区间、筛选车次、查询初始化摘要 | `engine_hooks._query_route_text()` | 不得使用 `至`、全角圆括号或 `【】` |

核心任务展示示例：`深圳北➜广州南、东莞➜海口`。
详情页路线文本通过 `#view-detail .detail-fact>b .route-detail-text{color:inherit}` 继承正文色；说明文字继续使用弱化色。
查询日志示例：`[G5154(马踏➜深圳)]`；不带车次号时使用 `[马踏➜深圳]`。
...
- 提交消息使用 `<type>: <简短主题>`；一次提交同时包含新功能和修复时使用 `feat+fix: <简短主题>`。
- 常用类型：`feat` 新功能、`fix` 修复、`docs` 文档、`refactor` 重构、`test` 测试、`chore` 工具或维护、`perf` 性能。
- 主题使用中文或英文均可，应准确说明结果，保持简短，不以句号结尾；不要写 `update`、`修改若干问题` 这类无法判断内容的主题。
- 一个提交只解决一个相互关联的主题；功能代码、无关格式化和临时调试文件不要混在同一提交中。
- 需要说明原因、影响范围或验证方式时写提交正文，正文与主题之间空一行；不要在提交中包含密钥、Cookie、`runtime/*` 数据、日志、缓存或临时脚本。
- 提交前至少检查 `git diff --check`，并运行与改动范围匹配的语法检查、测试或接口验证。

示例：

```text
feat+fix: 完善任务调度与站点校验

- 说明任务到点启动和服务恢复后的补启动行为
- 说明站点过滤和任务状态修复
- 说明已运行的验证命令
```
...
禁止直接使用未经检查的 `git pull` 作为默认流程。拉取和合并必须拆成「获取、阅读、审查、合并、验证」五步：

1. **获取前确认状态**：执行 `git status --short --branch`，记录当前分支、HEAD 和未提交改动。用户已有的修改、未跟踪文件或 `.github/hooks/` 不得擅自删除、覆盖、暂存或提交。
2. **只获取不自动改工作区**：执行 `git fetch origin`，然后确认目标远端分支和当前分支关系：
  `git log HEAD..origin/<branch> --oneline --decorate`。
3. **读取完整更新内容**：必须查看远端提交消息和统计：
  `git log HEAD..origin/<branch> --format=fuller --stat`；同时读取实际代码差异：
  `git diff HEAD...origin/<branch>`。改动较大时按文件继续读取关键实现、调用方、路由、数据库迁移、前端入口和测试，不得只看提交标题就合并。
4. **执行一次代码审查并熟悉更新**：合并前记录更新范围和审查结论，重点检查行为回归、数据迁移、鉴权、线程/生命周期、异常处理、兼容性和测试覆盖。发现明确问题时先停止合并并修复或报告；不能以「远端已有提交」为理由跳过审查。
5. **合并**：确认审查通过后优先执行 `git merge --ff-only origin/<branch>`。若本地和远端已分叉，不得擅自 `reset --hard`、丢弃本地提交或无说明地 rebase；先查看双方提交和差异，再选择明确的合并方案。出现冲突时逐文件阅读上下文，解决后检查 `git diff --check`，不确定时保留冲突并报告，不要猜测性删除代码。

### 13.3 合并后验证

- 执行 `git status --short --branch` 和 `git log --oneline --decorate -6`，确认 HEAD、远端跟踪关系和工作区状态。
- 执行 `git diff --check`；前端改动至少运行 `node --check py12306/webx/static/main.js`，Python 改动至少运行对应文件的语法检查或项目已有的检查命令。
- 涉及后端接口、数据库、任务调度、账号或订单链路时，必须做对应的真实 API、数据库查询、日志或最小行为验证；不能只凭编译通过下结论。
- 汇报合并结果时说明：远端提交范围、主要更新内容、代码审查结论、验证命令及结果，以及仍保留的未跟踪或未提交文件。

### 13.4 推送与提交边界

- 未经用户明确要求，不执行 `git push`，也不创建或删除分支。
- 不把用户未要求的未跟踪文件加入提交；不为了让工作区变干净而删除它们。
- 不修改、压缩或重写已有提交历史，除非用户明确要求，并先说明影响范围。
- 当前项目的运行时文件位于 `runtime/*`，一律不提交；临时脚本按 §9.2 处理并在验证结束后清理。
